...
import { withAuthentication } from "../security/withAuthentication";
import { type RequestHandler, withLogging } from "./logginUtils";
...
export const jsonResponseOr404 = <T, TRequest extends BunRequest>(
  ...
  return withLogging(
    withAuthentication(async (req: TRequest): Promise<Response> => {
      try {
